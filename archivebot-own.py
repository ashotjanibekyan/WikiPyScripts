...
from pywikibot.textlib import (
    ...
    extract_sections,
    findmarker,
    to_local_digits, TimeStripperPatterns,
)
from pywikibot.time import MW_KEYS, parse_duration, str2timedelta, TZoneFixedOffset
...
class DiscussionThread:
    ...
    def __init__(self, title: str, timestripper: MyTimeStripper) -> None:
        """Initializer."""
        self.title = title
...
class DiscussionPage(pywikibot.Page):
    ...
    def __init__(self, source, archiver, params=None, keep=False) -> None:
        ...
        if archiver is None:
            self.timestripper = MyTimeStripper(self.site)
        else:
...
class PageArchiver:
    ...
    def __init__(self, page, template, salt: str, force: bool = False,
        ...
        self.site = page.site
        self.tpl = template
        self.timestripper = MyTimeStripper(site=self.site)
        # read maxarticlesize
        try:
...
class MyTimeStripper(pywikibot.textlib.TimeStripper):
    def __init__(self, site=None) -> None:
        ...
        functions = [pywikibot.tools.first_upper, pywikibot.tools.first_lower]
        for n, (long, short) in enumerate(self.site.months_names, start=1):
