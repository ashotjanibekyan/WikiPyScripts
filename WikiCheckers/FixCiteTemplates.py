    ...
    def execute(self, text: str, parsed: mwparserfromhell.wikicode) -> (str, str):
        ...
        for template in parsed.filter_templates(recursive=True):
            ...
            for temp in self.cite_templates:
                ...
                self.replace_param(template, 'auteur1', 'auteur1', summaries)
                self.replace_param(template, 'auteur2', 'auteur2', summaries)
                self.replace_param(template, 'auteur3', 'auteur3', summaries)
                self.replace_param(template, 'auteur4', 'auteur4', summaries)
